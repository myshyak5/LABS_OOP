...
class Angle {
    ...
    public:
        ...
        Angle operator/(double divisor) const {
            //if (divisor == 0) throw std::invalid_argument("/0 :(");
        ...
        friend Angle operator+(double rad, const Angle& angle) {
            return Angle(rad + angle.m_rad);
        }
        friend Angle operator-(double rad, const Angle& angle) {
            return Angle(rad - angle.m_rad);
        }
        ...
        std::vector<AngleRange> operator+(const AngleRange& other) const {
            ...
            if (this->contains(other.m_start) || this->contains(other.m_end) || other.contains(m_start) || other.contains(m_end)) {
                ...
                if (m_start < other.m_start) {
                    start = m_start;
                    new_in_start = m_in_start;
                }
                else {
                    start = other.m_start;
                    new_in_start = other.m_in_start;
                }
...
bool operator<(const AngleRange& range1, const AngleRange& range2) {
    return range1.contains(range2);
}
bool operator<(const Angle& angle, const AngleRange& range) {
    return range.contains(angle);
}
int main() {
    Angle a1 = Angle::from_degrees(45);
    Angle a2 = Angle::from_radians(PI / 4);
    ...
    AngleRange range1(0, Angle::from_degrees(90), true, false);
    ...
    std::cout << "range1: " << range1.repr() << std::endl;
    std::cout << "range2: " << range2.repr() << std::endl;
    std::cout << "range1 length: " << range1.length() << " rad" << std::endl;
    std::cout << "range2 length: " << range2.length() << " rad" << std::endl;
    
    std::cout << "a1 in range1: " << (a1 < range1) << std::endl;
    std::cout << "a1 in range2: " << (a1 < range2) << std::endl;

    std::vector<AngleRange> range1p2 = range1 + range2;
    std::vector<AngleRange> range2p1 = range2 + range1;
    std::vector<AngleRange> range1m2 = range1 - range2;
    std::vector<AngleRange> range2m1 = range2 - range1;
    std::cout << "range1 + range2: ";
    for (const auto& range : range1p2) { std::cout << range.str() << (range1p2.size() > 1 ? " + " : " "); }
    std::cout << std::endl;
    std::cout << "range2 + range1: ";
    for (const auto& range : range2p1) { std::cout << range.str() << (range2p1.size() > 1 ? " + " : " "); }
    std::cout << std::endl;
    std::cout << "range1 - range2: ";
    for (const auto& range : range1m2) { std::cout << range.str() << (range1m2.size() > 1 ? " + " : " "); }
    std::cout << std::endl;
    std::cout << "range2 - range1: ";
    for (const auto& range : range2m1) { std::cout << range.str() << (range2m1.size() > 1 ? " + " : " "); }
    std::cout << std::endl;
